...
unique_Base GameScene::Update(unique_Base own)
{
	...
	if (IpSceneMng._chipNo.first == CHIP_TYPE::ínê}5)
	{
	...
	if (IpSceneMng._chipNo.first == CHIP_TYPE::ínê}1)
	{
		if (count < 5)
		{
			if (IpSceneMng.frames() % 180 == 0)
			{
				...
				if (IpSceneMng._data[672 / 32][32 / 32] != 4)
				{
					for (int x = 0; x < 1; x++)
					{
						for (int y = 0; y < 1; y++)
						{
							EnemyAdd(ENEMY_TYPE::ÉRÉEÉÇÉä, _objList, { 32 ,672 }, { 32,32}, { 1.0f,1.0f });
						}
					}
				}
...
void GameScene::MapInit(void)
{
	_Init.try_emplace(CHIP_TYPE::ínê}1, [&]() {
		...
		for (int x = 0; x < ChipMax.x * ChipMax.y; x++)
		{
			...
			{
				Flrdata = { FLOOR_TYPE::ìñÇΩÇËîªíË,{blocksize * (x % ChipMax.x),blocksize * (x / ChipMax.x)},{32,32} };
			...
			{
				Flrdata = { FLOOR_TYPE::ÉÅÉjÉÖÅ[,{blocksize * (x % ChipMax.x),blocksize * (x / ChipMax.x)},{32,32} };
				_objList.emplace_back(new Floor(Flrdata));
			}
	...
	_Init.try_emplace(CHIP_TYPE::ínê}2 , [&]() {
		...
		for (int x = 0; x < ChipMax.x * ChipMax.y; x++)
		{
			...
			if (IpSceneMng._dataBase[x] == 3)
			{
				...
				Itmdata = { ITEAM_TYPE::ÉÅÉjÉÖÅ[,{blocksize * (x % ChipMax.x),blocksize * (x / ChipMax.x)},{32,32} };
			...
			{
				EnemyAdd(ENEMY_TYPE::ÉIÅ[ÉN, _objList, { blocksize * (x % ChipMax.x),blocksize * (x / ChipMax.x) }, { 32,32}, { 1.0f,1.0f });
			}
	...
	_Init.try_emplace(CHIP_TYPE::ínê}4, [&]() {
		...
		for (int x = 0; x < ChipMax.x * ChipMax.y; x++)
		{
			...
			{
				Itmdata = { ITEAM_TYPE::ÉXÉEÉBÉbÉ`,{blocksize * (x % ChipMax.x),blocksize * (x / ChipMax.x)},{32,32} };
				_objList.emplace_back(new Iteam(Itmdata));
			}
	...
	_Init.try_emplace(CHIP_TYPE::ínê}5, [&]() {
		...
		IpImageMng.GetID("É{ÉX", "image/Monsters/boss.png", {120,120}, { 3,4 });
		//IpImageMng.GetID("ÉÇÉìÉXÉ^Å[ï‡Ç≠", "image/Monsters/walk.png", { 32,32 }, { 3,32 });
		...
		for (int x = 0; x < 1; x++)
		{
			for (int y = 0; y < 1; y++)
			{
				EnemyAdd(ENEMY_TYPE::É{ÉX, _objList, { 384 ,32 }, { 120,120 }, { 1.0f,1.0f });
			}
		}
	...
	_Draw.try_emplace(CHIP_TYPE::ínê}5, []() {
...
void GameScene::ChangeInit(void)
{
	...
	_Change.try_emplace(CHIP_TYPE::ínê}5, [&]() {
