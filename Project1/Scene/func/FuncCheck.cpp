		...
		for (auto obj : ((GameScene*)scene)->_objList)
		{
			if ((obj->_unitID == unitID) && (*obj).isAlive())
			{
				...
				case DIR_ID::UP:
					...
					if (CheckBox({ S_pos(actQue.second.posGet(), actQue.second.sizeGet()).x,S_pos(actQue.second.posGet(), actQue.second.sizeGet()).y- actQue.second.sizeGet().y/2 }, actQue.second.sizeGet(), (*obj).posGet(), (*obj).sizeGet(), TYPE::¶ã))
					{
				...
				case DIR_ID::LEFT:
					...
					if (CheckBox({ S_pos(actQue.second.posGet(), actQue.second.sizeGet()).x - actQue.second.sizeGet().x / 2,S_pos(actQue.second.posGet(), actQue.second.sizeGet()).y  }, actQue.second.sizeGet(), (*obj).posGet(), (*obj).sizeGet(), TYPE::¶ã))
					{
						(*obj)._status[Status_ID::HP] -= 100;
						return true;
					}
